...
public class Solver {
    ...
    static int evals = 0;
    static Annealer anneal = new Annealer();
    public static void main(String[] args) {
        try {
            ...
            BigDecimal elapsedTime = new BigDecimal(System.nanoTime() - startTime);
            System.out.printf("\nExec Time: %.2fs", elapsedTime.divide(new BigDecimal(1000000000)));
            System.out.printf("\nBox Pushes: %d", i);
            System.out.printf("\nEvaluations: %d", evals);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    // public static boolean isFrozen(char[][] mapData, char[][] itemsData, Coord boxMoved, Coord goals) {
    //     boolean exit = false;
    //     Coord cur = boxMoved;
    //     char axis = 'h';
    //     while (exit == false) {
    //         Coord[] UDLR = cur.getUDLRCoords();
    //         if (axis == 'h') {
    //             // check for horizontal movement
    //             // check for blocking walls
    //             if (!BoxMove.isOpen(mapData[UDLR[0].r][UDLR[0].c]) || !BoxMove.isOpen(mapData[UDLR[1].r][UDLR[1].c])
    //             || )
    //         } else if (axis == 'v') {
